...
$module = (Read-Xml "$script:RepoRoot/Modules/DragonslayerGutsPowers/SubModule.xml").Module
if ($module.Id.value -cne 'DragonslayerGutsPowers' -or $module.SingleplayerModule.value -ne 'true') { throw 'Invalid add-on identity/scope.' }
...
if ($module.SubModules.SubModule.DLLName.value -cne 'DragonslayerGutsPowers.dll' -or $module.SubModules.SubModule.SubModuleClassType.value -cne 'DragonslayerGutsPowers.SubModule') { throw 'Invalid add-on entry point.' }
